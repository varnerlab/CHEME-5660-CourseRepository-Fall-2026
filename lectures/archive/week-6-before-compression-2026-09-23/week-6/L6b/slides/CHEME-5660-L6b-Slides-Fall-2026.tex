...
\begin{frame}{Separation, and What Breaks It}
  With one financing rate and no risk-free-leg or dollar constraints, investors
  sharing the inputs hold one tangent fund and vary only $w_f$ (Tobin, L5b).
  Long-only bounds on the risky proportions preserve this result. What breaks it:
  \begin{itemize}\setlength{\itemsep}{0.15em}
    \item \hilite{No borrowing} ($w_f\ge0$): truncates the CAL at $\Tan$; above
      it the efficient boundary generally follows the risky frontier. The package
      does not impose this restriction.
    \item \hilite{Position bounds} ($w_i\le u$): kink the ray where they bind and
      curb extreme, mean-sensitive weights; robustness must be tested.
    \item \hilite{Higher borrowing rate}: a fixed fund gets a flatter ray at
      $\Tan$; reoptimization generally yields another borrowing tangent, with a
      risky-frontier segment between the two.
  \end{itemize}
...
\begin{frame}{Summary}
  ...
  \begin{itemize}\setlength{\itemsep}{0.2em}
    ...
    \item \hilite{One risky answer for every risk-free fraction}: a free $w_f$
      ...
      bounds or no borrowing kink or truncate the ray, while a higher borrowing
      rate generally introduces a second tangent risky fund.
