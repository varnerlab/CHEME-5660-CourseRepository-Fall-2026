...
\begin{frame}{Gross Notional Value Is About 108 Times Premium Paid}
  \vspace{0.05em}
  In 2025, buyers paid an average of \hilite{USD 37B} in option premiums each
  trading day. Those contracts had a much larger \hilite{gross notional value}.

  \vspace{0.25em}
  Standard equity options generally represent \textbf{100 shares}; index options
  use a contract multiplier. Across 60.4 million contracts per day, gross
  notional was about \hilite{USD 4 trillion}:
  \[
  ...
  \vspace{0.05em}
  This ratio approximates exposure-based leverage when $|\delta| \approx 1$, as
  for a deep in-the-money call. Otherwise, delta determines market exposure
  and therefore leverage. The \hilite{USD 4 trillion} is gross notional, not
  cash turnover.

  \slidenote{Careful:}{108 times is gross notional divided by premium; it approximates leverage only when $|\delta| \approx 1$. About USD 3.0T comes from cash-settled S\&P 500 index options; no shares are transferred.}
\end{frame}
