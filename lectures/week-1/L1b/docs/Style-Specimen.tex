...
\begin{figure}[ht]
  ...
  \input{../figs/abstract-asset-timeline.tex}
  \caption{The abstract asset $\mathcal{A}$ comprises a signed net cash flow $\bar c_j$ at each index $j\in\{0,1,\ldots,N\}$, where $N=nT$ is the terminal cash-flow index. Following the process-flow convention, an arrow entering a time-point unit is money received and an arrow leaving it is money paid. The sign is carried by $\bar c_j$ itself, so the labels are written without an additional minus. Discounting transports each future cash flow to the common valuation date $j=0$.}
  \label{fig:abstract-asset}
\end{figure}

To construct the required conversion rule, begin with the simplest transport problem. Suppose the initial principal $P$ dollars earns the nominal annual yield $y$ for one year. The resulting year-end future value $F$ is given by \eqnref{annual-compounding}:
\begin{equation}
...
\end{equation}
where the $\star$ subscript denotes the ordered $(N+1)$-vector over $j=0,\ldots,N$. Every term in this sum is measured in time-0 currency units, so NPV is a single scalar product.
\end{definition}

The sign of NPV compares the asset with the benchmark encoded in the discount curve. It is a valuation statement conditional on the declared cash-flow model and yield, not a guarantee of realized profit or of outperforming any particular alternative:
...
\textbf{Negative NPV ($\operatorname{NPV}<0$).}\enspace Discounted payments exceed discounted receipts, so under the stated assumptions the asset does not clear the benchmark. This does not by itself imply a nominal cash loss: undiscounted receipts may still exceed undiscounted payments once time is ignored.
