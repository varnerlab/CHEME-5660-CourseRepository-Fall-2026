...
%   Discrete Compounding       [cell 5]  -> Discrete Compounding
...
\begin{frame}{Lectures and Examples}
  ...
  \vspace{0.55em}
  Derive accumulation and discount factors, represent an asset as dated signed
  cash flows, and compute NPV at a declared valuation rate.
...
\begin{frame}{Discrete Compounding}
  ...
  $i_\ell>-1$ be the \mbox{\hilite{effective rate}} over the interval
...
\begin{frame}{What Valuation Rate Should I Use?}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.47\textwidth}
      \eyebrow{Rate type}
      \vspace{0.25em}
      {\small A quoted \textbf{\ink{bond yield}} is the rate that matches price
      to promised cash flows. An \textbf{\ink{equity required return}} estimates
      future compensation for risk. They are different quantities.}
    \end{column}
    \begin{column}{0.47\textwidth}
      \eyebrow{Risk type}
      \vspace{0.25em}
      {\small Only \textbf{\ink{systematic risk}}, which diversification cannot
      remove, is expected to earn compensation. More diversifiable risk does not
      by itself increase expected return. Expected return is not realized return.}
    \end{column}
  \end{columns}
\end{frame}
