...
\begin{frame}{Review: Yields and Accumulation Factors}
  ...
  \vspace{0.4em}
  Let $P$ be an amount held today, $y$ a quoted \hilite{nominal annual yield},
  and $n$ the compounding intervals per year. At period index $j$, the
  equivalent future amount is:
  \[
...
\begin{frame}{Review: Abstract Assets and Net Present Value}
  ...
  each must be converted to a common valuation date before the values are added.

  \vspace{0.5em}
  Let $N$ be the final cash-flow index and $\bar c_j$ the \hilite{signed net cash
  flow} at index $j$. Under a constant nominal yield $y$, the time-0 net present
  value is:
  \[
...
\begin{frame}{The NPV Sign Is a Valuation Test}
  With every cash flow in today's dollars, NPV compares the present value
  received with the amount paid.
  ...
  \vspace{0.3em}
  \hilite{The sign is conditional on the discount model and assumed cash flows,
...
\begin{frame}{Treasury Bills}
  ...
  \begin{itemize}\setlength{\itemsep}{0.55em}
    \item \textbf{\ink{Regular Treasury bills}} have standardized 4-, 6-, 8-,
      13-, 17-, 26-, or 52-week terms on a recurring auction schedule, bought
      through TreasuryDirect or a bank, broker, or dealer.
    \item \textbf{\ink{Cash Management Bills (CMBs)}} have variable terms from a
      few days to one year, are auctioned only when Treasury needs them, and
      must be bought through a bank, broker, or dealer.
  \end{itemize}
...
\begin{frame}{Two Ways to Bid at a T-Bill Auction}
  ...
  \begin{itemize}\setlength{\itemsep}{0.7em}
    \item \textbf{\ink{Noncompetitive bid:}} Most individual investors request a
      ...
      limit, the award is guaranteed. TreasuryDirect offers only this bid type.
    \item \textbf{\ink{Competitive bid:}} Institutional and other experienced
      ...
      they will accept: control over the minimum return, at the risk of a
      partial or no award. These bids go through a bank, broker, or dealer.
  \end{itemize}
...
\begin{frame}{A Competitive Bid Controls Allocation, Not Price}
  ...
  $10{,}000~\mathrm{USD}$ and a $60\%$ proration is $6{,}000~\mathrm{USD}$:
  \hilite{award amounts, not cash prices}.
...
\begin{frame}{Valuing a T-Bill as an Abstract Asset}
  Let $y$ be a nominal annual valuation yield, $n$ its compounding frequency,
  and $g_y=n\log(1+y/n)$. Setting NPV to zero gives:
  \[
  ...
  For the model price, $g_B=g_y$; for an observed market price, $g_B$ is implied
  by that price and may differ from the model input.
...
\begin{frame}{Treasury Notes and Bonds}
  ...
  \vspace{0.5em}
  At original issue, semiannual payments give $n=2$ and $N=nT$ coupon dates; in
  a reopening or secondary-market trade, $N$ counts the remaining coupons. With
  annual coupon rate $c$, each coupon is $C=(c/n)V_P$:
  \[
    \bar c_0=-V_B,
    \qquad
...
\begin{frame}{The Auction Yield Sets the Coupon and Price}
  The highest competitive yield to receive an award, the \hilite{high yield}
  $y^*$, determines the common auction price: bids below $y^*$ receive the full
  requested par, bids at $y^*$ may be prorated, and bids above it receive
  nothing.
...
\begin{frame}{Valuing Notes and Bonds as Abstract Assets}
  Value on a coupon date with $N$ complete coupon periods remaining, and let $y$
  be the nominal annual YTM compounded semiannually. Setting NPV to zero and
...
\begin{frame}{Sovereign Debt Can Default or Be Restructured}
  Government debt is not inherently default-free. Two sovereign examples changed
  the timing or amount of promised payments:
  ...
  \begin{itemize}\setlength{\itemsep}{0.65em}
    \item \textbf{\ink{Russia, 1998:}} On August 17, Russia announced a de facto
      ...
      creditor losses amplified an international financial and liquidity crisis.
  ...
  \vspace{0.35em}
  Both the probability of default and the recovery after default affect a
  security's value.
...
\begin{frame}{Optional Advanced Material}
  ...
  coupon payments with Bernoulli survival indicators, adds recovery after
