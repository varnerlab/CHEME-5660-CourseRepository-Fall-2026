...
\begin{frame}{Growth Rates and Log Returns}
  ...
  $S_1,\ldots,S_T$, the cumulative log return is:
  \[
    \sum_{t=2}^{T}r_t
...
\begin{frame}{Excess Growth Rate}
  ...
  both in inverse years. For an interval of length $\Delta t$ years:
  \[
...
\begin{frame}{Algorithm}
  ...
  \begin{enumerate}\setlength{\itemsep}{0.6em}
    \item For each $t=2,\ldots,T$, read consecutive prices
      $S_{t-1}^{(i)}$ and $S_t^{(i)}$.
    \item Compute the excess growth rate:
      \[
        \bar g_t^{(i)}\gets
...
\begin{frame}{Risk}
  ...
  prices. Their mean $g^{\prime}$ and sample volatility $\sigma_g$ are:
  \[
...
\begin{frame}{Log-Return and Annualized Volatility}
  ...
  Its standard deviation $\sigma_r(\Delta t)$ is:
  \[
  ...
  The conventional annualized value $\sigma_{\mathrm{ann}}$ is:
  \[
...
\begin{frame}{Power-Law Tails}
  ...
  A regularly varying tail satisfies:
  \[
    \mathbb P(X>x)\sim
  ...
  The factor $L(x)$ is slowly varying: for every fixed $c>0$,
  \[
