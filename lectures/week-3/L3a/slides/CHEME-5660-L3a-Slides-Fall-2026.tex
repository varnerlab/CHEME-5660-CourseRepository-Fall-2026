...
\begin{frame}{\large Heavy-Tailed Growth-Rate Distributions}
  ...
  \begin{itemize}\setlength{\itemsep}{0.7em}
    ...
    \item \hilite{Student-$t$}: power-law tails. Smaller degrees of freedom
      $\nu$ give heavier tails. The true tail index is $\alpha=\nu$.
  \end{itemize}
...
\begin{frame}{Power-Law Tails}
  ...
  A smaller $\alpha$ means heavier tails and more extreme events.
  Student-$t$ tails follow this form; Normal and Laplace tails do not.
\end{frame}

\begin{frame}{Why the Tail Index Matters}
  Smaller estimated $\alpha$ means heavier tails and more extreme events.

  \vspace{0.6em}
  We simulated 5,000 samples per distribution, each with 2,766 observations,
  and used the 52 largest absolute movements to estimate $\alpha$.
  ...
  \slidenote{Simulation:}{independent observations, mean zero, equal variance.
    Ranges depend on sample size and cutoff. Overlap means an estimate alone
    cannot identify the distribution.}
\end{frame}

\begin{frame}{True and Estimated Tail Indices}
  For Student-$t$, the true tail index is $\alpha=\nu$.
  The value can be fractional.

  \vspace{0.9em}
  \begin{itemize}\setlength{\itemsep}{0.8em}
    \item \hilite{True index}: a Student-$t$ model with $\nu=4$ has
      $\alpha=4$.
    \item \hilite{Estimated index}: our simulation gives a median Hill
      estimate of $\widehat\alpha=3.5$ for that same model.
    \item Estimates depend on the sample and how many extreme observations
      we include.
  \end{itemize}

  \slidenote{Example:}{the fitted Student-$t$ parameter uses all observations.
    Hill uses only the largest magnitudes. Their estimates can differ.}
\end{frame}
