...
\lecturenotes{4a}{Equity Markets and the Stylized Facts of Returns}{Fall 2026}{September 15, 2026}{Jeffrey D. Varner}
...
\learningobjectives
  ...
  {Compute returns}
  {Calculate simple, log, and excess returns with consistent price, dividend, time, and compounding conventions.}
...
\section{Price Changes Are Not Yet Returns}

A price change in dollars depends on the initial share price and ignores distributions. A return rescales the investment outcome and must specify whether dividends are included. Let $S_{t-1}>0$ and $S_t>0$ denote ex-dividend share prices in dollars per share at consecutive dates, and let $D_t\geq0$ be the cash distribution per share received between them. The holding-period return and its logarithmic counterpart are defined in \defnref{return-measures}.

\begin{definition}{Simple and logarithmic returns}{return-measures}
Let $S_{t-1}>0$ and $S_t>0$ be share prices in dollars per share at dates $t-1$ and $t$, and let $D_t\geq0$ dollars per share be a distribution received during the interval. The simple total return $R_t$ and log total return $g_t$ are
\begin{equation}
  \label{eq:return-measures}
  R_t:=\frac{S_t+D_t-S_{t-1}}{S_{t-1}},
  \qquad
  g_t:=\log(1+R_t)=\log\!\left(\frac{S_t+D_t}{S_{t-1}}\right),
\end{equation}
provided $1+R_t>0$. When $D_t=0$, the price log return is $g_t=\log(S_t/S_{t-1})$. If the interval length is $\Delta t>0$ years, the continuously compounded growth rate is $\mu_t:=g_t/\Delta t$ in inverse years.
\end{definition}
...
Log returns are especially useful because they add across nonoverlapping time intervals (\propref{log-additivity}). Simple returns compound multiplicatively instead.

\begin{proposition}{Aggregation of price log returns}{log-additivity}
Let $S_0,S_1,\ldots,S_T$ be strictly positive share prices measured at increasing dates, and suppose distributions are absent or already incorporated consistently into a total-return index. Define the one-period price log return by $g_t:=\log(S_t/S_{t-1})$ for $t\in\{1,\ldots,T\}$. Then the cumulative log return satisfies
\begin{equation}
  \label{eq:log-additivity}
  \sum_{t=1}^{T}g_t
  =\log\!\left(\frac{S_T}{S_0}\right),
\end{equation}
while the cumulative simple return satisfies $1+R_{0,T}=\prod_{t=1}^{T}(1+R_t)$.
\end{proposition}
...
An excess log return compares the equity outcome with a benchmark over the same interval. If $r_{f,t}$ is the continuously compounded benchmark rate in inverse years, then $x_t:=g_t-r_{f,t}\Delta t$. This is a relative performance measure, not proof that the benchmark itself is literally riskless.
...
The empirical notebook calculates these diagnostics for common stocks and ETFs. Its role is model criticism: the output should be compared with the assumptions and implications of the models introduced in L4b and L5b.

\notebooklink
  {L4a: Stylized Facts of Stocks and ETFs}
  {https://github.com/varnerlab/CHEME-5660-CourseRepository-Fall-2026/blob/main/lectures/week-4/L4a/CHEME-5660-L4a-StylizedFacts-Example-Fall-2026.ipynb}
  {Compute empirical return distributions, autocorrelations, tail diagnostics, and volatility-clustering evidence from historical prices.}
...
\notebooklink
  {L4a: SVD of S\&P 500 Growth Rates}
  {https://github.com/varnerlab/CHEME-5660-CourseRepository-Fall-2026/blob/main/lectures/week-4/L4a/CHEME-5660-L4a-SVD-SP500-Example-Fall-2026.ipynb}
