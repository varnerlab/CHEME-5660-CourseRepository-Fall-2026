...
\begin{frame}{Lectures and Examples}
  ...
  drift, and compare simulated price paths with the data.

  \vspace{1.1em}
  \textbf{\ink{Example:}}
  \href{https://github.com/varnerlab/CHEME-5660-CourseRepository-Fall-2026/blob/main/lectures/week-5/L5a/CHEME-5660-L5a-Example-GBM-NPV-TradeRule-Fall-2026.ipynb}{GBM Terminal-Target Probabilities}
...
\begin{frame}{Company Profile: Jane Street}
  ...
  \begin{itemize}\setlength{\itemsep}{0.5em}
    ...
    \item \hilite{Technology}: \href{https://www.janestreet.com/technology/}{Jane Street uses OCaml}, a functional programming
      language, in its trading systems.
  \end{itemize}

  \vspace{0.25em}
  \textbf{Explore further:}
  \href{https://www.youtube.com/@janestreet}{YouTube},
  \href{https://www.janestreet.com/join-jane-street/internships/}{internships}, and
  \href{https://www.janestreet.com/join-jane-street/open-roles/}{open positions}.

  \vspace{0.25em}
  We use models to describe uncertain future prices and calculate
  probabilities of trade outcomes.
\end{frame}

\begin{frame}{From Lattices to Continuous Prices}
  In L4a, we generalized the binomial lattice to an N-ary lattice with
  $m$ possible price movements at each step. Let's extend two ideas:

  \vspace{0.5em}
  \begin{itemize}\setlength{\itemsep}{0.6em}
    \item \hilite{What prices can occur?} Tomorrow's price can take one of
      $m$ values. We can extend this to any positive price, using a continuous
      distribution to calculate probabilities for price ranges.
    \item \hilite{When can the price change?} The holding period is the time
      between buying and selling. Keeping those dates fixed, shorten the steps
      and use smaller typical price movements. Letting $\Delta t\to0$ allows
      us to describe prices in continuous time.
  \end{itemize}

  \vspace{0.5em}
  \hilite{GBM} supplies a rule for continuous price paths: log returns over
  non-overlapping intervals are independent and normally distributed.
  Future prices are therefore lognormal and positive.
\end{frame}
...
\begin{frame}{Single Asset Geometric Brownian Motion (GBM)}
  ...
  \vspace{0.5em}
  The drift controls expected price growth; volatility controls the size of
  random fluctuations. Let's define the Wiener process.
\end{frame}

\begin{frame}{The Wiener Process}
  Named after \href{https://en.wikipedia.org/wiki/Norbert_Wiener}{Norbert Wiener},
  a Wiener process $\{W(t),\,0\le t\le T\}$ is a continuous stochastic
  ...
  \vspace{0.5em}
  Each increment has the same distribution as a scaled standard normal:
  \[
    W(t)-W(s)\overset{d}{=}\sqrt{t-s}\;Z,\qquad Z\sim\mathcal{N}(0,1).
  \]
  We call $Z$ a \hilite{random shock}, the standard normal input to the model.
  In simulation, each step uses a fresh independent draw.
\end{frame}
...
\begin{frame}{Volatility}
  ...
  Here, $\hat\sigma$ estimates the unknown GBM parameter $\sigma$. It matches
  L3a's annualized volatility $\sigma_{\mathrm{ann}}$, with units year$^{-1/2}$.
...
\begin{frame}{Mean Growth and Drift}
  The mean growth rate $\mug=\mu-\sigma^2/2$ is the growth term in the
  one-step solution. We can use its estimate directly to simulate prices:
  \[
    S_{t_j}=S_{t_{j-1}}\exp\!\left[
      \hat{\mu}_g\Delta t+\hat\sigma\sqrt{\Delta t}\,Z_j\right].
  \]
  The hats indicate estimates from data. To express the fitted model using
  the drift $\mu$ in the original differential equation, calculate:
  \[
    \boxed{\hat\mu=\hat{\mu}_g+\frac{\hat\sigma^2}{2}.}
  \]
  Substituting this estimate into the one-step solution cancels the correction:
  \[
    \hat\mu-\frac{\hat\sigma^2}{2}
    =\left(\hat{\mu}_g+\frac{\hat\sigma^2}{2}\right)
      -\frac{\hat\sigma^2}{2}=\hat{\mu}_g.
  \]
  Both forms give the same prices for the same draws of $Z_j$.
\end{frame}

\begin{frame}{Estimating Mean Growth}
  Random price fluctuations can make mean growth difficult to estimate from
  a finite historical sample. We consider two methods:
  ...
  \vspace{0.7em}
  To use regression, we need an equation relating each log price to its
  observation time. Let's build that equation from the one-step solution.
\end{frame}

\begin{frame}{Linear Regression: Steps 1 and 2}
  Start from $S_0$ at time zero and observe prices every $\Delta t$ years.
  Taking logarithms of the first update gives:
  \[
    \ln S_{t_1}=\ln S_0+\mug\Delta t+\sigma\sqrt{\Delta t}\,Z_1.
  \]
  At step 2, we start from the price reached at step 1:
  \[
    \ln S_{t_2}=\ln S_{t_1}+\mug\Delta t+\sigma\sqrt{\Delta t}\,Z_2.
  \]
  Substitute the first update and collect terms:
  \[
    \begin{aligned}
      \ln S_{t_2}
      &=\left[\ln S_0+\mug\Delta t+\sigma\sqrt{\Delta t}\,Z_1\right]
        +\mug\Delta t+\sigma\sqrt{\Delta t}\,Z_2\\
      &=\ln S_0+\mug(2\Delta t)+\sigma\sqrt{\Delta t}\,(Z_1+Z_2).
    \end{aligned}
  \]
  The second log price contains the random contribution from step 1
  as well as the new contribution from step 2.
\end{frame}

\begin{frame}{Linear Regression: The Model}
  After $j$ steps, with $t_j=j\Delta t$, the log price is given by:
  \[
    \ln S_{t_j}
    =\underbrace{\ln S_0}_{\text{intercept}}
    +\underbrace{\mug\,t_j}_{\text{trend}}
    +\underbrace{\epsilon_j}_{\text{error}},
    \qquad
    \epsilon_j=\sigma\sqrt{\Delta t}\sum_{k=1}^{j}Z_k,\quad \epsilon_0=0.
  \]
  Each $Z_k$ has mean zero. Thus $\E[\epsilon_j]=0$, and the expected log
  price is a line with slope $\mug$. Fitting a line to observed log prices
  gives us an estimate of mean growth.

  \vspace{0.5em}
  The draws $Z_k$ are independent, but the regression errors share draws:
  \[
    \epsilon_1=\sigma\sqrt{\Delta t}\,Z_1,
    \qquad \epsilon_2=\sigma\sqrt{\Delta t}\,(Z_1+Z_2).
  \]
  The shared $Z_1$ makes these errors correlated. Under GBM,
  $\Var(\epsilon_j)=\sigma^2t_j$ also grows with time.
\end{frame}
...
\begin{frame}{Regression Intervals: Standard Errors}
  \hilite{Engineering assumption (a spherical cow):} Treat the regression
  errors as independent and normal, with mean zero and common variance
  $\sigma_\epsilon^2$, to illustrate the ordinary uncertainty calculation.
...
\begin{frame}{Regression Intervals: Confidence Intervals}
  Under the spherical cow assumption, a confidence interval
  ...
  \vspace{0.4em}
  These intervals do not provide valid GBM uncertainty bounds because the
  log-price errors are correlated and their variance increases with time.
  ...
  uses the correlated errors $\epsilon_j$, each a scaled sum of independent
  $Z_k$ draws, to assess slope uncertainty and its effect on target probabilities.
...
\begin{frame}{The Closed-Form Target Probability}
  ...
  \end{itemize}
  \slidenote{Example:}{\href{https://github.com/varnerlab/CHEME-5660-CourseRepository-Fall-2026/blob/main/lectures/week-5/L5a/CHEME-5660-L5a-Example-GBM-NPV-TradeRule-Fall-2026.ipynb}{GBM Terminal-Target Probabilities}}
\end{frame}
