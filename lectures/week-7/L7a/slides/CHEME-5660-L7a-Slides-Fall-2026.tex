...
\begin{frame}{Concept Review: Allocation as a Time-Stamped Decision}
  ...
  growth-rate vector and covariance; with the SIM of L6a those inputs are
  $\gSIM=\hat{\bal}+\hat{\bbe}\,g'_M$ and
  $\SigSIM=s^2_{g,M}\hat{\bbe}\hat{\bbe}^{\top}+\hat{\mathbf D}_g$
  ($\hat{\mathbf D}_g$: residual variances).
  ...
  \]
  \hilite{Two things to say out loud}: the weights are optimal only for the
  inputs estimated on the decision date, and the investor's one decision was at
  $t=0$. Today is about the other days.
\end{frame}
...
\begin{frame}{Cobb--Douglas Utility: Sign Classifies, Magnitude Allocates}
  ...
  \]
  The \hilite{dollar weight} of a preferred asset is
  its normalized preference weight $\gamma_i/\sum_{j\in\Ap}\gamma_j$, independent
  of prices. An empty basket means floors plus cash.
\end{frame}
...
\begin{frame}{Preference Weights: The Basket, and Assumptions Stated}
  \begin{itemize}\setlength{\itemsep}{0.45em}
    ...
    \item If every $\gamma_i(t)\le0$, the model is saying \hilite{this basket does
      ...
      basket (the ticker-picking methods of L12b and L13a).
...
\begin{frame}{The Rebalancing Engine: The Account and the Timing}
  ...
  day $t$ use prices through $t-1$; the trade executes at day $t$'s close. A
  close-based signal traded at that close is look-ahead.
\end{frame}
...
\begin{frame}{Algorithm: Rebalancing Engine (Backtest), Daily Loop}
  ...
  \begin{enumerate}\setlength{\itemsep}{0.1em}
    ...
    \item Breaker: if the drawdown exceeds $d_{\max}$ with risky positions held,
      ...
      more days, go to 5.
    \item If $t\in\Rset$ and not locked: ask for $\bw^\star$; form the proposed
      change of weights, scale to $\tau_{\max}$.
    \item Solve cost and positions together on the post-cost budget; execute
      $\Delta n_i$ at $S_i(t)$; pay $C_t=c\,Q_t$; update the cash.
    \item Record wealth, cash, positions, weights, turnover, cost, and flags.
  \end{enumerate}
