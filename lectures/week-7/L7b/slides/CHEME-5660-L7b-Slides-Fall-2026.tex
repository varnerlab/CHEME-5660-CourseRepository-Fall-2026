...
\begin{frame}{Concept Review: The Batch SIM Fit and the Engine}
  ...
  ($g$: annualized daily growth rates, $\varepsilon$: the residual, $t=1,\ldots,N$
  training days), by least squares: with response vector $\by$, design matrix
  $\bX$ (row $t$: $\bx_t^{\top}=(1,g_{M,t})$), and $\bth_i=(\alpha_i,\beta_i)^{\top}$:
  \[
...
\begin{frame}{Why Frozen Parameters Can Fail: The Consequences}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3em}
    \item \hilite{The threshold} $-\alpha_i/\beta_i$: the frozen thresholds sit
      ...
      year (its sampling error at any short memory) moves the threshold by more.
    ...
    \item \hilite{The covariance matrix} $\SigSIM=s^2_{g,M}\hat{\bbe}\hat{\bbe}^{\top}+\hat{\mathbf D}_g$
      ($\hat{\bbe}$ the betas, $\hat{\mathbf D}_g$ the diagonal of residual
      variances): with $s^2_{g,M}$ held fixed, the online betas and residual scales
...
\begin{frame}{EWLS: Weights, Loss, Decay Factor, and Half-Life}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35em}
    \item An observation $\thalf$ days old carries half today's weight; the total
      ...
      count of weight, not the variance-based effective sample size).
...
\begin{frame}{The Scenario Ensemble: The Distributional Scorecard}
  ...
  holds the market and residual draws fixed and leaves the policy contrast.
\end{frame}
