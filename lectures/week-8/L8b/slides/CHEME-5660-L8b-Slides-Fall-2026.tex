...
\begin{frame}{Composite Positions Add Leg by Leg}
  ...
  Both are in USD/share. For account dollars, multiply leg $i$ by its
  deliverable $q_i\in\N$ shares/contract.
  \begin{itemize}\setlength{\itemsep}{0.2em}
...
\begin{frame}{Composite Positions: Three Structures, One Sum}
  ...
  \begin{itemize}\setlength{\itemsep}{0.02em}
    ...
    \item \hilite{Straddle}: a call and a put at one strike $K$, same direction.
      Profit $\theta\bigl(\lvert\ST-K\rvert-D\bigr)$; breakevens $K+D$, and $K-D$ when $D\leq K$.
    \item \hilite{Strangle}: a call and a put at strikes $K_1<K_2$, same
      direction. A flat plateau replaces the straddle's single kink.
  \end{itemize}
  \vspace{-0.35em}
  \textbf{\ink{State the bounds correctly.}} A long straddle is often said to have
  ``unlimited profit in either direction.'' \hilite{That is wrong on the downside.}
  Since $\ST\geq0$ the put leg cannot pay more than its strike: the AMD $K=230$
  straddle with $D=42.88$ caps its downside at $187.12$ USD/share.
